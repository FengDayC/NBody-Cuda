 ...
 */
__global__
void calculateForce(float* buf)
{
    GET_INFO

        Body* bodyBuf = (Body*)buf;

    int bodyIndex = blockPartId * THREADSPERBLOCK + threadBlockId;
    ...
    Body body = bodyBuf[partId * BODIESPERPART + threadBlockId];
...
#pragma unroll 64
    ...
    for (int j = 0; j < THREADSPERBLOCK; j++) {
        float px = __shfl_sync(0xffffffff, body.x, j);
        float py = __shfl_sync(0xffffffff, body.y, j);
        float pz = __shfl_sync(0xffffffff, body.z, j);
        float dx = px - myBody.x;
        float dy = py - myBody.y;
        float dz = pz - myBody.z;
    ...
    if (partId == PARTCNT - 1)
    {
        atomicAdd(&bodyBuf[bodyIndex].x, bodyBuf[bodyIndex].vx * dt);
        atomicAdd(&bodyBuf[bodyIndex].y, bodyBuf[bodyIndex].vy * dt);
        atomicAdd(&bodyBuf[bodyIndex].z, bodyBuf[bodyIndex].vz * dt);
    }
}

__global__
void calculatePos(float* buf)
{
    GET_INFO
...
int main(const int argc, const char** argv) {
    ...
    for (int iter = 0; iter < nIters; iter++) {
        ...
        /*******************************************************************/
             ...
             */
        calculateForce << < {PARTCNT* BLOCKPERPART, 1, 1}, { THREADSPERBLOCK,1,1 } >> > (device_buf);

        cudaDeviceSynchronize();

        /*calculatePos << < {BLOCKPERPART, 1, 1}, {THREADSPERBLOCK,1,1} >> > (device_buf);
